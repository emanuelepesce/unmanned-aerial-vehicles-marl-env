...
class Scenario(BaseScenario):
    def make_world(self):
        ...
        world.alpha = 1.e-2
        world.beta = 1.e-3
        # if this attribute exists move the landmarks(the value doesn't matter)
        world.moving_landmarks = True
        # Min and Max offsets used to move the landmark in a funciton like this:
        # np.random.uniform(self.world.landmark_movex,self.world.landmark_movey, 2)
        world.landmark_move_min = -0.3 
        world.landmark_move_max = 0.3
    ...
    def move_landmarks(self, world):
        for i, landmark in enumerate(world.landmarks):
            landmark.state.p_pos += np.random.uniform(0.1, 0.1, 2)