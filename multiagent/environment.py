...
class MultiAgentEnv(gym.Env):
    ...
    def _step(self, action_n):
        ...
        # move landmarks if required
        if hasattr(self.world, 'moving_landmarks'):
            for i, landmark in enumerate(self.world.landmarks):
                landmark.state.p_pos += np.random.uniform(self.world.landmark_move_min,self.world.landmark_move_max, 2)
    ...
    def _render(self, mode='human', close=True, render_vision = True):
        ...
        for i in range(len(self.viewers)):
            ...
            if self.viewers[i] is None:
                ...
                from multiagentsha import rendering
        ...
        if self.render_geoms is None:
            ...
            for entity in self.world.entities:
                if entity.typec=='landmark':
                    geom = rendering.make_polygon_landmark()
                elif entity.typec=='agent':
                    # geom = rendering.make_circle(entity.size)
                    if hasattr(entity, 'max_vision') and render_vision:
                        geom = rendering.make_circle_vision(radius = entity.size, radius_vision=entity.size+entity.max_vision)
                        xform = rendering.Transform()
                        geom.set_color_vision(*entity.color, alpha=0.5)
