...
class EntityState(object):
    def __init__(self):
        ...
        self.p_pos = None
        # physical velocity (true airspeed!)
        self.p_vel = None
        # energy (proxy of battery)
        self.energy = None
...
class Landmark(Entity):
     def __init__(self,color=np.array([1, 1, 0])):
...
class World(object):
    @property
    def wind_components(self) -> np.array:
        dx = math.cos(math.radians(self.wind_direction))
        dy = math.sin(math.radians(self.wind_direction))
        return np.array([dx, dy])

    def __init__(self):
        # list of agents and entities (can change at execution-time!)
        # wind speed (in m/s) and direction (in deg)
        self.wind_speed = 0.
    ...
    def distance_to_target(self, agent):
        agent_pos = agent.state.p_pos
        agent_target_pos = self.landmarks[agent.target].state.p_pos
        return np.sqrt(np.sum(np.square(agent_pos - agent_target_pos)))
    ...
    def integrate_state(self, p_force):
        for i,entity in enumerate(self.entities):
            ...
            if self.distance_to_target(entity) < entity.size * 2:
                # has reached the target
                entity.state.p_vel = np.zeros(self.dim_p)
                entity.collide = False
            else:
                entity.state.energy -= (self.alpha * np.linalg.norm(entity.action.u) + self.beta) * self.dt
                entity.state.energy = max(0., entity.state.energy)

                # agent has not energy to move (it is in the ground), continue without effect on position nor speed
                if entity.state.energy == 0.:
                    entity.state.p_vel = np.zeros(self.dim_p)
                    entity.collide = False
                else:
                    # agent is still flying
                    ...
                    # update position considering speed and wind
                    wind = self.wind_components * self.wind_speed
                    entity.state.p_pos += (entity.state.p_vel + wind) * self.dt
